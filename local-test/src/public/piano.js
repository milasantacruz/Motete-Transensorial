...
function drawHeader() {
  ...
  // Estado MQTT
  fill(mqttConnected ? [0, 150, 0] : [200, 0, 0]);
  textSize(14);
  text(`MQTT: ${mqttConnected ? 'Conectado' : 'Desconectado'}`, 20, 55);
...
function drawPianoBoard(board) {
  ...
  // Indicador único de configuración (mismo valor para todas las teclas)
  const cfgRoot = osmoConfigs[board.osmoId] || {};
  let anyCfg = null;
  // tomar la primera config disponible
  ...
  if (anyCfg) {
    const actSec = Math.round(anyCfg.activationTime / 100) / 10; // 1 decimal
    const cdSec = Math.round(anyCfg.cooldownTime / 100) / 10;
    ...
    fill(200);
    // Dibujar: [timer][actSec s]  [autorenew][cdSec s] alineado a la derecha
...
async function loadStatus() {
  try {
    ...
    // Actualizar estado MQTT en el DOM
    const mqttStatus = document.getElementById('mqtt-status');
    if (mqttStatus) {
      mqttStatus.textContent = mqttConnected ? '✅ Conectado' : '❌ Desconectado';
    }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (responseData.success) {
        console.log(`${mode}Comando enviado a ${unitId}: bomba ${pumpId} - EXITOSO`);
        // Activar LED y configurar temporizador
        activateKeyLED(unitId, pumpId, duration);
      } else {
        console.log(`${mode}Comando rechazado por ${unitId}: bomba ${pumpId} - ${responseData.message || 'Error desconocido'}`);
